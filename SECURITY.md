...
2. **默认零外发**：不访问任何外部域名；唯一网络行为是页面到本机 dsh web 的 GET 路由，且自 v0.2.1 起要求配对头 `x-dsh-security-doctor: 1` 防跨站读取。**唯一显式例外（v0.3.0 起，受控变更并公示）**：用户手动点击报告界面的「检查更新」按钮时，浏览器向 `api.github.com` 发起一次查询本插件最新 Release 的 GET——只读版本信息，不上行任何数据；默认关闭、无自动后台检查、除该按钮外代码中不存在任何外部域名。

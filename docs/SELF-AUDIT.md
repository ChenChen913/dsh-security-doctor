...
>
> 审查对象：v0.2.1 ｜ 审查方式：静态源码审查 + 测试断言 ｜ 结论：**未发现 T1–T10 命中项；自审另发现 3 处可加固点（S1–S3），已全部修复并覆盖测试。**
...
| T5 数据外发 | ✅ 无命中 | 全库无外部域名；fetch 仅本机相对路径路由；无遥测 |
...
| **网络** | 页面 → 本机 dsh web 的两个 GET（`/check`、`/self-test`），带头、`no-store`。**零外部域名** |

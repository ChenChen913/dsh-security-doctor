...
| --- | --- | --- |
| `!!js` in config | Every cordis patch/config under `~/.dsh`, comments and `\|`/`>` literal blocks stripped (doc samples don't count); `!!js` is evaluated at load | High |
...
| Plugin egress scan | Static scan of external plugin source for `http(s)://`/`ws(s)://` hosts (comments stripped, loopback excluded), per plugin; `eval`/base64/raw `node:net`·`node:dns` obfuscation signals annotated as first-pass markers; plugins without scannable source flagged | Info/Attention |
| Credential file permissions | `~/.dsh/.credentials.yaml`: POSIX group/other bits (0400/0600 pass); **Windows: read-only `icacls` ACL query, broad accounts (Users/Everyone plus their well-known SIDs and localized names) flagged**. Bits/ACL only, contents never read | Attention |
| Workspace instruction files | `AGENTS.md` / `CLAUDE.md` / `GEMINI.md` / `.github/copilot-instructions.md` / `.agents/` / `.cursor/rules/` with SHA-256 hashes, diffed against the last checkup | Info |
...
The `#v0.7.1` tag pins the exact released commit (reproducible, rollback-able; once published to npm: `dsh plugin --profile web add dsh-security-doctor`). Running DSH from a source checkout? From the harness repo: `pnpm dsh plugin --profile web add github:ChenChen913/dsh-security-doctor#v0.7.1`, or `node --import tsx/esm apps/cli/src/bin.ts plugin --profile web add github:ChenChen913/dsh-security-doctor#v0.7.1`.

Install self-verification (curl returns `ok:true` + console logs `[dsh-security-doctor] client loaded; host self-test: v0.7.1` + the button appears):
...
2. **Zero egress by default**: localhost GET routes require the pairing header `x-dsh-security-doctor: 1` (cross-site pages cannot read your report); since v0.7.0 the `Host` header must also name a local address, blocking same-origin-spoofed reads under DNS rebinding (since v0.7.1, LAN / reverse-proxy deployments can extend the whitelist via the `DSH_ALLOWED_HOSTS=host1,host2` environment variable — local-only by default); no external domain exists in the code except the manual check-update query.
...
| --- | --- | --- | --- |
| v0.7.1 | DSH 0.1.0-rc.5 | Windows (source run) | Feedback-fix release: localized-Windows ACL (SIDs/zh-CN), YAML literal blocks, obfuscation signals, instruction-file candidates, DSH_ALLOWED_HOSTS; macOS/Linux via CI matrix; Node ≥ 22 |

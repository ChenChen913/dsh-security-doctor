  ...
  factory: (require) => {
    ...
    var STR = {
      ...
      en: {
        ...
        checking: 'Checking…', upToDate: 'Up to date', newVersion: 'Update available', updateHow: 'Update steps: see the "Update" section in README', noRelease: 'No published release found (the repo may not have published one yet)', updateFailed: 'Update check failed (network or GitHub unreachable)',
      },
    };
     ...
     */
    var PATH_RE = /([A-Za-z]:\\[\w.\-\u4e00-\u9fff]+(?:\\[\w.\-\u4e00-\u9fff]+)*|~\/[\w.\-\u4e00-\u9fff]+(?:\/[\w.\-\u4e00-\u9fff]+)*|(?:[\w.-]+\/){2,}[\w.-]+)/g;
          ...
          var run = function (auto) {
            ...
            fetch('/dsh-security-doctor/check', { method: 'GET', headers: { 'x-dsh-security-doctor': '1' } })
              ...
              .then(function (data) {
                ...
                // user finding 3.2-5: the mount-time auto checkup only pops
                // the modal open when there is an unacknowledged high finding;
                // otherwise the result stays in the badge and waits for a
                // click, so refreshing the page no longer re-opens the report
                setPhase(!auto || (data.report.summary && data.report.summary.high > 0) ? 'open' : 'idle');
              })
              .catch(function (err) {

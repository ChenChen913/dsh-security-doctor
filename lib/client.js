  ...
  factory: (require) => {
    ...
    var STR = {
      zh: {
        ...
        // v1.1.0 (UX review): guard mode deserves a visible plain-language
        // briefing, not a hover-only title — users were left guessing what
        // it does and whether it keeps running after DSH exits (it does not)
        guardAlways: '仅 DSH 运行时生效 · 关闭即停 · 卸载无残留',
        guardWhatIs: '这是什么？', guardHelpTitle: '守护模式说明',
        guardHelpBody: '开启后，插件帮你多盯着两件事：① 哪个插件往哪个网站发了请求（只记网站名，不记内容）；② 你的配置文件、指令文件被改动时提醒你。',
        guardHelpQ1: '会拖慢电脑吗？—— 几乎不占资源。',
        guardHelpQ2: '会把数据发出去吗？—— 不会。记录只存在本机内存里，刷新页面即消失。',
        guardHelpQ3: '关掉 DSH 后它还在后台运行吗？—— 不会。它不是系统服务，DSH 一关闭就彻底停止、不留任何残留。',
        guardHelpQ4: '为什么默认关闭？—— 实验特性，等你需要时再开。',
        // v1.1.0 (UX review): conclusion-first egress card — three answers
        // per critical plugin (what/why/what-to-do), advisory + raw listing
        // collapse behind plain-language toggles
        egWhy: '为什么', egWhat: '怎么办', egFiles: '涉及文件',
        egAdvisoryLead: '个插件带提醒信号（绝大多数是正常功能）',
        egAdvisoryShow: '展开提醒', egAdvisoryHide: '收起提醒',
        egRawShow: '查看完整技术清单', egRawHide: '收起技术清单',
        egRawNote: '技术明细，供专业人员核查', egSelfNote: '本插件说明',
      ...
      en: {
        ...
        // v1.1.0 (UX review): guard-mode plain-language briefing (see zh twin)
        guardAlways: 'runs only while DSH is up · off stops it · uninstall leaves nothing behind',
        guardWhatIs: 'What is this?', guardHelpTitle: 'Guard mode, explained',
        guardHelpBody: 'When on, the plugin watches two extra things for you: (1) which plugin sends requests to which website (site names only, never contents); (2) alerts you when your config or instruction files change.',
        guardHelpQ1: 'Will it slow my machine down? — Negligible.',
        guardHelpQ2: 'Does it send data out? — No. Records live in local memory only and vanish on refresh.',
        guardHelpQ3: 'Does it keep running after I close DSH? — No. It is not a system service; the moment DSH exits it stops completely and leaves nothing behind.',
        guardHelpQ4: 'Why is it off by default? — It is experimental; turn it on when you want it.',
        // v1.1.0 (UX review): conclusion-first egress card (see zh twin)
        egWhy: 'why', egWhat: 'what to do', egFiles: 'files involved',
        egAdvisoryLead: 'plugin(s) carry advisory signals (routine in most cases)',
        egAdvisoryShow: 'Show advisories', egAdvisoryHide: 'Hide advisories',
        egRawShow: 'Full technical listing', egRawHide: 'Hide technical listing',
        egRawNote: 'Technical detail for professionals', egSelfNote: 'About this plugin itself',
    ...
    function CheckCard(props) {
      ...
      // v1.1.0 (UX review): conclusion-first tiers for the egress card
      // (FIX-PLAN-v1.1.0 §三.A). Layer 1 critical blocks are always visible;
      // layer 2 advisories fold behind their own toggle; layer 3 raw listing
      // rides the existing `expanded` state. Reports from older plugin
      // versions (no tiers field) keep the classic render untouched.
      var tiers = check.id === 'plugin-egress' && check.extra && check.extra.tiers ? check.extra.tiers : null;
      var advOpenState = React.useState(false);
      var advOpen = advOpenState[0];
      var setAdvOpen = advOpenState[1];
      // when tiers ride the report, prose[0] is the engine's lead conclusion
      // line ("⚠ N 个插件命中高危组合…"). It gets its own styled element at
      // the top of the card, so the raw-list prose skips it (and the blank
      // separator after it) to avoid double rendering.
      var tieredProse = tiers
        ? prose.slice(1).filter(function (l) { return l.trim() !== '' })
        : prose;
      var proseList = tieredProse.length > 0 ? React.createElement('ul', { key: 'list', className: 'dsd-check__list' },
        tieredProse.map(function (line, i) {
      ...
      // v1.1.0 (UX review): the tiered body — WHAT is risky / WHY / WHAT to
      // do in plain language, then fold-out layers. The classic body stays
      // for old reports without tiers. The lead line (prose[0], e.g. "⚠ N
      // 个插件命中高危组合…") gets its own emphasized element.
      function criticalBlock(c, i) {
        var entry = susMap[c.name];
        var deepBtn = entry && hasMarkers(entry)
          ? React.createElement('button', {
            key: 'dr', type: 'button', className: 'dsd-mini dsd-mini--ghost', title: t('deepReviewNote'),
            onClick: function () {
              copyText(buildDeepReviewPrompt(entry), props.onToast
          ...
          : null;
        return React.createElement('div', { key: 'crit' + i, className: 'dsd-critical' }, [
          React.createElement('div', { key: 'head', className: 'dsd-critical__head' }, [
            React.createElement('span', { key: 'n', className: 'dsd-critical__name' }, c.name),
            deepBtn,
          ]),
          React.createElement('p', { key: 'why', className: 'dsd-critical__why' },
            [React.createElement('span', { key: 'l', className: 'dsd-critical__label' }, t('egWhy') + SEP.c), c.why]),
          React.createElement('p', { key: 'what', className: 'dsd-critical__what' },
            [React.createElement('span', { key: 'l', className: 'dsd-critical__label' }, t('egWhat') + SEP.c), c.what]),
        ]);
      }
      var tieredBody = null;
      if (tiers) {
        var tiered = [
          ...
          prose.length > 0
            ? React.createElement('p', { key: 'lead', className: 'dsd-check__lead' }, prose[0])
            : null,
        ];
        // layer 1: critical three-liners (always visible)
        if (Array.isArray(tiers.critical)) {
          for (var ci = 0; ci < tiers.critical.length; ci++) tiered.push(criticalBlock(tiers.critical[ci], ci));
        }
        if (tiers.selfNote) {
          tiered.push(React.createElement('p', { key: 'self', className: 'dsd-check__selfnote' }, tiers.selfNote));
        }
        // layer 2: advisories behind their own toggle
        if (Array.isArray(tiers.advisory) && tiers.advisory.length > 0) {
          var advRows = tiers.advisory.map(function (a) {
            return React.createElement('li', { key: a.name, className: 'dsd-advisory__row' },
              [React.createElement('span', { key: 'n', className: 'dsd-advisory__name' }, a.name),
                React.createElement('span', { key: 's', className: 'dsd-advisory__sum' }, a.summary)]);
          });
          tiered.push(React.createElement('div', { key: 'adv', className: 'dsd-advisory' }, [
            React.createElement('button', {
              key: 'btn', type: 'button', className: 'dsd-advisory__toggle', onClick: function () { setAdvOpen(!advOpen) },
              'aria-expanded': advOpen ? 'true' : 'false',
            }, (advOpen ? '▾ ' : '▸ ') + tiers.advisory.length + ' ' + t('egAdvisoryLead')
              + SEP.s + (advOpen ? t('egAdvisoryHide') : t('egAdvisoryShow'))),
            advOpen ? React.createElement('ul', { key: 'list', className: 'dsd-advisory__list' }, advRows) : null,
          ]));
        }
        // layer 3: the raw technical listing behind the shared expanded state
        tiered.push(React.createElement('button', {
          key: 'raw', type: 'button', className: 'dsd-mini', onClick: function () { setExpanded(!expanded) },
        }, expanded ? t('egRawHide') : t('egRawShow')));
        if (expanded) {
          tiered.push(React.createElement('p', { key: 'rawnote', className: 'dsd-check__rawnote' }, t('egRawNote')));
          tiered.push(proseList);
          tiered.push(metaList);
        }
        tiered = tiered.concat(instrNotes, treeNotes, sessionNotes, [
          check.status !== 'pass' ? React.createElement('p', { key: 'advice', className: 'dsd-check__advice' }, t('advice') + check.advice) : null,
        ]);
        tiered = tiered.filter(function (n) { return n !== null && n !== undefined });
        tieredBody = React.createElement('div', { key: 'main', className: 'dsd-check__main' }, tiered);
      }
      ...
      return React.createElement('div', {
        ...
        tieredBody || React.createElement('div', { key: 'main', className: 'dsd-check__main' }, [
          React.createElement('span', { key: 'title', className: 'dsd-check__title', title: check.title }, check.title),
          proseList,
          metaList,
    ...
    function ReportModal(props) {
      ...
      // v1.1.0 (UX review): the "What is this?" briefing card — guard mode
      // had only a hover title; users could not tell what it does nor
      // whether it survives closing DSH (it does not). Folded by default.
      var guardHelpState = React.useState(false);
      var guardHelpOpen = guardHelpState[0];
      var setGuardHelpOpen = guardHelpState[1];
      ...
      return React.createElement('div', { className: 'dsd-overlay', onClick: onClose }, [
        React.createElement('div', {
          ...
          report ? React.createElement('div', { key: 'footer', className: 'dsd-modal__footer' }, [
            ...
            // v1.1.0 (UX review): a visible one-liner replaces hover-only
            // discovery, plus a plain-language briefing card — the switch
            // answered "what does it do" and "does it keep running after
            // DSH exits" for nobody before.
            React.createElement('div', { key: 'guardbar', className: 'dsd-guardbar' }, [
              ...
              React.createElement('button', {
                key: 'what', type: 'button', className: 'dsd-guardbar__what',
                onClick: function () { setGuardHelpOpen(!guardHelpOpen) },
                'aria-expanded': guardHelpOpen ? 'true' : 'false',
              }, t('guardWhatIs')),
              React.createElement('span', { key: 'always', className: 'dsd-guardbar__always' }, t('guardAlways')),
            ]),
            guardHelpOpen ? React.createElement('div', { key: 'guardhelp', className: 'dsd-guardhelp' }, [
              React.createElement('p', { key: 't', className: 'dsd-guardhelp__title' }, t('guardHelpTitle')),
              React.createElement('p', { key: 'b', className: 'dsd-guardhelp__body' }, t('guardHelpBody')),
              React.createElement('ul', { key: 'q', className: 'dsd-guardhelp__qa' }, [
                React.createElement('li', { key: 'q1' }, t('guardHelpQ1')),
                React.createElement('li', { key: 'q2' }, t('guardHelpQ2')),
                React.createElement('li', { key: 'q3' }, t('guardHelpQ3')),
                React.createElement('li', { key: 'q4' }, t('guardHelpQ4')),
              ]),
            ]) : null,
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => {
        ...
        tag.textContent = [
          ...
          // v1.1.0 (UX review): conclusion-first egress card — the lead
          // conclusion, critical three-liner blocks, advisory fold, raw note
          '.dsd-check__lead{margin:2px 0 0;font-size:13.5px;line-height:22px;font-weight:600}',
          '.dsd-critical{margin-top:10px;padding:10px 12px;border-radius:12px;border:1px solid rgba(127,127,127,.18);background:rgba(255,255,255,.4);background:color-mix(in srgb,var(--dsw-alias-bg-layer-1,#fff) 40%,transparent);display:flex;flex-direction:column;gap:6px}',
          '.dsd-critical__head{display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap}',
          '.dsd-critical__name{font-size:13.5px;line-height:20px;font-weight:600;word-break:break-all}',
          '.dsd-critical__why,.dsd-critical__what{margin:0;font-size:13px;line-height:21px;word-break:break-word}',
          '.dsd-critical__label{font-weight:600;margin-right:6px}',
          '.dsd-critical__what{color:var(--dsw-alias-label-primary,#1f1f1f)}',
          '.dsd-check__selfnote,.dsd-check__rawnote{margin:8px 0 0;font-size:12px;line-height:19px;color:var(--dsw-alias-label-secondary,#5f6368)}',
          '.dsd-advisory{margin-top:8px}',
          '.dsd-advisory__toggle{border:none;padding:0;background:none;cursor:pointer;font-size:12.5px;line-height:20px;color:var(--dsw-alias-label-secondary,#454747);text-align:left}',
          '.dsd-advisory__toggle:hover{color:var(--dsw-alias-label-primary,#1f1f1f)}',
          '.dsd-advisory__list{margin:6px 0 0;padding:0;list-style:none;display:flex;flex-direction:column;gap:5px}',
          '.dsd-advisory__row{font-size:12.5px;line-height:20px;word-break:break-word}',
          '.dsd-advisory__name{font-weight:600;margin-right:6px}',
          '.dsd-advisory__sum{color:var(--dsw-alias-label-secondary,#454747)}',
          // v1.1.0 (UX review): guard-mode briefing — visible one-liner + the
          // "What is this?" card in the report footer
          '.dsd-guardbar__always{font-size:11.5px;line-height:18px;color:var(--dsw-alias-label-tertiary,#80868b)}',
          '.dsd-guardbar__what{border:none;padding:0;background:none;cursor:pointer;font-size:11.5px;line-height:18px;color:var(--dsw-alias-label-secondary,#5f6368);text-decoration:underline dotted}',
          '.dsd-guardbar__what:hover{color:var(--dsw-alias-label-primary,#1f1f1f)}',
          '.dsd-guardhelp{margin-top:8px;padding:12px 14px;border-radius:14px;border:1px solid rgba(127,127,127,.16);background:rgba(255,255,255,.32);background:color-mix(in srgb,var(--dsw-alias-bg-layer-1,#fff) 34%,transparent);display:flex;flex-direction:column;gap:6px}',
          '.dsd-guardhelp__title{margin:0;font-size:13px;line-height:20px;font-weight:600}',
          '.dsd-guardhelp__body{margin:0;font-size:12.5px;line-height:20px}',
          '.dsd-guardhelp__qa{margin:0;padding-left:18px;display:flex;flex-direction:column;gap:4px}',
          '.dsd-guardhelp__qa li{font-size:12.5px;line-height:20px;color:var(--dsw-alias-label-secondary,#454747)}',

...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function CheckCard(props) {
      ...
      var metaList = meta.length > 0 ? React.createElement('div', { key: 'meta', className: 'dsd-check__meta' },
        meta.map(function (m, i) {
          return React.createElement('div', { key: 'm' + i, className: 'dsd-check__meta-row', title: m.text },
            m.from ? [t('source') + ' · '].concat(renderDetailLine(m.text, 'm' + i)) : renderDetailLine(m.text, 'm' + i));
        })) : null;
    ...
    function ReportModal(props) {
      ...
      var findings = sorted.filter(function (c) { return c.status !== 'pass' });
      var passes = sorted.filter(function (c) { return c.status === 'pass' });
      var bodyChildren = report ? [overview].concat(findings.map(function (check) {
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => {
        ...
        tag.textContent = [
          ...
          // source/metadata rows — quiet, single-line, ellipsized; the full
          // text stays available through the row title tooltip
          '.dsd-check__meta{display:flex;flex-direction:column;gap:3px}',
          '.dsd-check__meta-row{font-size:12px;line-height:18px;color:var(--dsw-alias-label-secondary,#5f6368);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}',

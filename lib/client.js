/**
 * dsh-security-doctor — client half (v0.2).
 *
  ...
  factory: (require) => {
    ...
    var STR = {
      zh: {
        ...
        footer: '尽力检测（best-effort），"未见异常"不等于绝对安全；深度检测请配合仓库 docs/ 下的《安全检测指南》。本插件只读、不外发数据。',
      ...
      en: {
        ...
        footer: 'Best-effort detection: "no findings" is not "safe". Deep review: see the security-review guide in the repo docs. This plugin is read-only and sends nothing anywhere.',
    ...
    function renderMarkdown(report) {
      ...
      lines.push('---', '', '尽力检测（best-effort）；由 dsh-security-doctor 生成。');
      return lines.join('\n');
    }
        ...
        React.createElement('div', {
          ...
          report ? React.createElement('div', { key: 'footer', className: 'dsd-modal__footer' }, [
            React.createElement('div', { key: 'note' },
              t('generatedAt') + ' ' + new Date(report.generatedAt).toLocaleString() + ' · ' + t('footer')),
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => {
        var disposeSlot = ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register({
          ...
          React.useEffect(function () {
            fetch('/dsh-security-doctor/self-test', { method: 'GET' })
              .then(function (res) { return res.json(); })

...
async function checkThirdPartyPlugins(home, pluginVersion) {
  let collected
  try {
  ...
  if (self && !hasOthers && self.pinned && !self.installScript) {
    return { id: 'third-party-plugins', title: '第三方插件盘点', severity: 'info', status: 'pass',
      detail: `除本插件自身（${SELF_NAME}${pluginVersion ? ' v' + pluginVersion : ''}，${self.spec}，已锁定、无安装脚本）外，未发现其他外来插件依赖。`,
      advice: '保持现状；安装新插件时（dsh plugin add）记得先用安全检测流程审一遍来源。',
  ...
  if (self) advice += ` 本插件自身的锁定安装方式：dependencies 写 "${SELF_REPO}${pluginVersion ? `#v${pluginVersion}` : '#<发版标签>'}"，消除未锁定提示。`
...
async function checkCredentialsFile(home, options) {
  ...
  if (platform === 'win32') {
    ...
    const label = (e) => `${/^S-1-\d/i.test(e.account) ? `${e.account}（未解析 SID）` : e.account}:${e.perms}`
 ...
 * @returns {Promise<{generatedAt: string, home: string, workspace: string, pluginVersion?: string, checks: CheckResult[], summary: object, verdict: string}>}
 */
export async function runSecurityCheckup(options) {
  const { home, workspace, services, env = process.env, platform, statFile, icacls, pluginVersion } = options
  const configFiles = await collectConfigFiles(home)

  const checks = [
    await guard('js-directives', '配置中的 !!js 表达式', () => checkJsDirectives(configFiles, home)),
    await guard('third-party-plugins', '第三方插件盘点', () => checkThirdPartyPlugins(home, pluginVersion)),
  ...
  const report = { generatedAt: new Date().toISOString(), home, workspace, checks, summary, verdict }

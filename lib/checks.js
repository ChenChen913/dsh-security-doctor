...
async function checkSecurityLayerPatches(configFiles, home, locale) {
  ...
  for (const file of configFiles.filter((f) => !f.endsWith('package.json'))) {
    ...
    while (i < lines.length) {
      const head = /^(\s*)-\s*(remove|replace)\s*:\s*(#.*)?$/.exec(stripYamlComment(lines[i]))
      if (!head) { i++; continue }
      const indent = head[1].length
      const kind = head[2]
      // collect the block body: everything indented deeper than the op line
      const body = []
      let j = i + 1
      while (j < lines.length) {
        const code = stripYamlComment(lines[j])
      ...
      for (const b of body) {
        ...
        if (kv && SEC_LAYER_RE.test(kv[1])) {
          hits.push(`${rel}:${i + 1}: ${kind} → ${maskSecrets(kv[1])}`)
        }
      }
      i = j
    }
...
async function checkInstructionFiles(workspace, locale) {
  ...
  const candidates = ['AGENTS.md', 'CLAUDE.md', 'CLAUDE.local.md', '.agents', '.cursor/rules', '.cursorrules', '.windsurfrules', '.clinerules']
  const found = []

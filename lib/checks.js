...
/**
 * v1.1.0 (UX review): documentation / spec / example hostnames (zod's docs
 * cite example.com ×57, json-schema.org ×177 …). They stay visible in the
 * raw technical listing, but no longer count as "plugins carrying outbound
 * signals" — otherwise the advisory count scares users over doc links.
 * Suffix-match covers subdomains; the bare-host split drops query residue
 * (`example.com?key=value` keys survive host extraction).
 */
const DOC_HOST_SUFFIXES = ['example.com', 'json-schema.org', 'w3.org', 'semver.org', 'iana.org']
function isDocHost(host) {
  const bare = String(host).split(/[?#]/)[0]
  return DOC_HOST_SUFFIXES.some((s) => bare === s || bare.endsWith('.' + s))
}
...
async function checkPluginEgress(home, locale) {
  ...
  const { external: externalAll } = collected
  // v1.1.0 (UX review): stop scanning OURSELVES. The doctor's own checks.js
  // legitimately contains child_process + 'curl' strings, which its own
  // combination rules flag as exec-channel — "the security plugin is high
  // risk" destroys the report's credibility. Honest handling: excluded from
  // the scan AND said so, not silently dropped.
  const external = externalAll.filter((d) => d.name !== SELF_NAME)
  const selfExcluded = externalAll.length !== external.length
  if (external.length === 0) {
    return { id: 'plugin-egress', title, severity: 'info', status: 'pass',
      detail: selfExcluded
        ? (en ? `No external plugins besides this plugin itself (self-audited publicly, see the repo's docs/SELF-AUDIT.md); nothing to scan.` : '除本插件自身（已按公开自审报告核查，见仓库 docs/SELF-AUDIT.md）外没有外来插件，无出网面可扫描。')
        : (en ? 'No external plugins; nothing to scan.' : '没有外来插件，无出网面可扫描。'),
      advice: en ? 'Nothing to do.' : '无需处理。' }
  }
  const COMBO_LABEL = {
  ...
  for (const dep of external) {
    ...
    perPlugin.push({
      name: dep.name,
      dir: dep.dir,
      // v1.1.0 doc-host split: doc/spec/example hosts still ride the raw
      // listing below (docHosts) but no longer count as outbound signal
      hosts: [...hosts.entries()].sort((a, b) => b[1] - a[1]).filter(([h]) => !isDocHost(h)).slice(0, 8),
      docHosts: [...hosts.entries()].sort((a, b) => b[1] - a[1]).filter(([h]) => isDocHost(h)).slice(0, 3),
  ...
  const lines = perPlugin.map((p) => {
    ...
    else if (p.docHosts && p.docHosts.length > 0) parts.push(`${en ? 'doc/example hosts only (muted): ' : '仅文档/示例域名（已降噪）：'}${p.docHosts.map(([h, n]) => `${h}${n > 1 ? `(×${n})` : ''}`).join(en ? ', ' : '、')}`)
  ...
  // ── v1.1.0 (UX review): conclusion-first tiers ─────────────────────────
  // The raw 60-line listing above is the auditor's worksheet. Most users
  // only ever needed three answers per problem: WHAT is risky, WHY, WHAT to
  // do. Tiers give the client those answers in plain language; the listing
  // stays as the drill-down layer for professionals. Plain-wording style
  // (user-approved): direct with a jolt, then the honest caveat — ability
  // co-occurrence, never a verdict.
  const COMBO_PLAIN = {
    'cred-exfil': en
      ? 'the SAME file contains credential-reading AND outbound-sending code — together, a complete pipeline for smuggling your secrets out'
      : '同一份文件里同时出现「读密钥」和「对外发送」的代码——拼起来就是一条把你的密钥偷运出去的完整通道',
    'exec-channel': en
      ? 'the SAME file contains system-command AND network-download code — together, enough to do nearly anything behind your back'
      : '同一份文件里同时出现「执行系统命令」和「联网下载」的代码——拼起来就能在幕后做几乎任何事',
    persistence: en
      ? 'the SAME file contains file-writing AND startup-related code — the complete chain for planting itself as permanent code'
      : '同一份文件里同时出现「写文件」和「开机自启」相关的代码——具备把自己变成常驻程序的完整链条',
  }
  const CAVEAT = en
    ? 'This is capability co-occurrence, not proof of wrongdoing — but it does not belong in a plugin you cannot vouch for.'
    : '这是能力共现，不等于一定在作恶——但说不清来源的插件不该带这种组合。'
  const ACTION = en
    ? 'Confirm you know this plugin and where it came from; if not, remove it.'
    : '确认你认识这个插件、知道它从哪来的；说不清就卸载它。'
  const critical = comboPlugins.map((p) => ({
    name: p.name,
    why: p.combos.map(([c]) => COMBO_PLAIN[c] ?? c).join(en ? '; ' : '；'),
    what: `${CAVEAT}${en ? ' ' : ''}${ACTION}`,
    files: p.comboFiles,
    combos: p.combos,
  }))
  // advisory: one plain sentence per plugin (single signals, obfuscation,
  // injection text, install scripts, real hostnames) — routine features in
  // most cases; shown collapsed by the client
  const advisory = perPlugin.filter((p) => !p.note && !(p.combos && p.combos.length > 0)
    && ((p.hosts && p.hosts.length > 0) || (p.suspicious && p.suspicious.length > 0)
      || (p.signals && p.signals.singles > 0) || (p.injection && p.injection.length > 0) || p.installScriptHits))
    .map((p) => {
      const bits = []
      if (p.hosts && p.hosts.length > 0) bits.push(`${en ? 'contacts ' : '会访问 '}${p.hosts.slice(0, 3).map(([h]) => h).join(en ? ', ' : '、')}`)
      if (p.signals && p.signals.envKeys && p.signals.envKeys.length > 0) bits.push(`${en ? 'reads secret-ish env keys (' : '读取密钥类环境变量（'}${p.signals.envKeys.slice(0, 2).join(', ')}${en ? ')' : '）'}${en ? ' — normal if it calls that service' : '——它本就要调用对应服务则属正常'}`)
      else if (p.signals && p.signals.credHits > 0) bits.push(en ? 'reads credential-like files' : '读取凭据类文件')
      if (p.signals && p.signals.emailSamples && p.signals.emailSamples.length > 0) bits.push(en ? 'email addresses in code (common in normal software)' : '代码里出现邮箱地址（正常软件也常有）')
      else if (p.signals && p.signals.mailHits > 0) bits.push(en ? 'mail-sending code' : '带邮件发送相关代码')
      if (p.suspicious && p.suspicious.length > 0) bits.push(en ? 'uses low-level net/process modules (static scan cannot tell why)' : '引入底层网络/进程模块（静态扫描看不出用途）')
      if (p.injection && p.injection.length > 0) bits.push(en ? 'prompt-injection markers in bundled text' : '随包文本中含提示词注入特征')
      if (p.installScriptHits) bits.push(`${en ? 'install script touches ' : '安装脚本会访问 '}${p.installScriptHits.text}`)
      if (bits.length === 0) bits.push(en ? 'minor signals, review only' : '轻微信号，仅供复核')
      return { name: p.name, summary: bits.join(en ? '; ' : '；') }
    })
  const tiers = {
    critical,
    advisory,
    cleanCount: external.length - critical.length - advisory.length - opaque.length,
    opaqueCount: opaque.length,
    selfNote: selfExcluded
      ? (en ? 'This plugin itself is excluded (self-audited publicly: docs/SELF-AUDIT.md in the repo).' : '本插件自身不列入扫描（已按公开自审报告核查：仓库 docs/SELF-AUDIT.md）。')
      : null,
  }
  // conclusion line rides detail for copy/export consumers; the raw listing
  // below it is unchanged
  const lead = critical.length > 0
    ? `${en ? `⚠ ${critical.length} plugin(s) hit high-risk combinations — confirm them NOW.` : `⚠ ${critical.length} 个插件命中高危组合，请立即确认。`}`
    : advisory.length > 0
      ? `${en ? `No high-risk combinations; ${advisory.length} plugin(s) carry advisory signals (routine in most cases).` : `未发现高危组合；${advisory.length} 个插件带提醒级信号（绝大多数是正常功能）。`}`
      : ''
